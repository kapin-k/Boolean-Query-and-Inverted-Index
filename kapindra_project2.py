...
def invertedIndex(corpus):
    ...
    for temp_dict in dictionary_sorted:
        ...
        #     if doc_id in inverted_index_WithFrequency[token].keys():
        #         frequncyCount = inverted_index_WithFrequency[token][doc_id]
        #         frequncyCount = int(frequncyCount) + 1
        #         inverted_index_WithFrequency[token][doc_id] = frequncyCount
        #     else:
        #         inverted_index_freq[doc_id] = 1
        #         inverted_index_WithFrequency[token] = inverted_index_freq
        #     print(inverted_index_WithFrequency)
        for token in token_gen:
...
def calculateAND_OR(query_line, inverted_index, outFile, sorted_dictionary):
    ...
    while no_list_empty:
        ...
        # DAAT APPROACH 2
        # min_length_posting_list = findMinimumLengthPosting(posting_list)
        # posting_list.remove(min_length_posting_list)
        # if (len(min_length_posting_list)>0) and (len(posting_list)>0):
        #     comparison_and = 0
        #     list_empty = True
        #     result_and = []
        #     while list_empty:
        #         delete_list = dict()
        #         found_in_all = True
        #         if(len(min_length_posting_list)>0):
        #             min_elem = min_length_posting_list[0]
        #         else:
        #             list_empty = False
        #             break
        #         delete_list['-1'] = min_elem
        #         for i in range(len(posting_list)):
        #             delete_list[i] = posting_list[i][0]
        #             if(min_elem == posting_list[i][0]):
        #                 comparison_and = comparison_and + 1
        #             else:
        #                 comparison_and = comparison_and + 1
        #                 found_in_all = False
        #         sorted_delete_list = sorted(delete_list.items(), key=lambda x:x[1])
        #         if(found_in_all):
        #             # print("min element"+str(min_elem))
        #             result_and.append(min_elem)
        #             min_length_posting_list.pop(0)
        #             for i in range(len(posting_list)-1):
        #                 del posting_list[i][0]
        #         else:
        #             simple_min = int(-1)
        #             if(len(sorted_delete_list) > 0):
        #                 for key,value in sorted_delete_list:
        #                     # print("simple min"+str(simple_min))
        #                     # print("key" + str(key))
        ...
        #                         if key == '-1':
        #                             min_length_posting_list.pop(0)
        #                             # print("minimum_length_posting _list is being popped")
        #                         else:
        #                             del posting_list[key][0]
        #             for list in posting_list:
        #                 if not list:
        #                     list_empty = False
        #                     break
        #             if not min_length_posting_list:
        #                 list_empty = False
        #                 break
